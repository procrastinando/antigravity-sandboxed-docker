...
# 3. THE FIX: Create a fake xdg-open that prints URLs to logs
RUN echo '#!/bin/sh' > /usr/bin/xdg-open && \
    echo 'echo "\n\n=================================================="' >> /usr/bin/xdg-open && \
    echo 'echo "PLEASE OPEN THIS URL IN YOUR BROWSER:"' >> /usr/bin/xdg-open && \
    echo 'echo "$1"' >> /usr/bin/xdg-open && \
    echo 'echo "==================================================\n\n"' >> /usr/bin/xdg-open && \
    chmod +x /usr/bin/xdg-open

    
# 4. Create a fake xdg-open to capture login URLs
RUN echo '#!/bin/sh' > /usr/bin/xdg-open && \
    echo 'echo "\n\n=================================================="' >> /usr/bin/xdg-open && \
    echo 'echo "PLEASE OPEN THIS URL IN YOUR BROWSER:"' >> /usr/bin/xdg-open && \
    echo 'echo "$1"' >> /usr/bin/xdg-open && \
